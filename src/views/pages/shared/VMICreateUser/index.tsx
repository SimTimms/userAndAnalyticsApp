  ...
  return (
    <div style={{ maxWidth: 400 }}>
      ...
      <DropDown
        ...
        placeholder="Scopes"
        data={[
          [UserTypes.Broker, 'Broker User'],
          [UserTypes.Admin, 'Admin User'],
          [UserTypes.VMI, 'VMI User'],
        ]}
        onClick={(id: string, name: string) =>
          HELPERS.handleClickChange(setFormData, formData, 'scope', id, true)
        }
      ></DropDown>
      ...
      <DropDown
        ...
        data={
          data && data.organisationMany
            ? data.organisationMany.map((item: any) => [item._id, item.name])
            : null
        }
        onClick={(id: string, name: string) =>
          HELPERS.handleClickChange(
            setFormData,
            formData,
            'organisation',
            id,
